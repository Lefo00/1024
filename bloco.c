...
void forSquare(int x, int y, Bloco bloco1)
{
    ...
    for(i = x; i < x + SQUARE_WIDTH; i++)
    {
		...
		if(bloco.valor <= 9){
           	 	printf("\n   %d", &bloco1.valor); //3 espaços e 1 linha, para centralizar(ou tentar)
		}else{
			if(bloco.valor <= 99){
				printf("\n  %d", &bloco1.valor); //2 espaços e 1 linha, para centralizar(ou tentar)
			}else{
				if(bloco.valor <= 999){
					printf("\n %d ", &bloco1.valor); //1 espaço e 1 linha, para centralizar(ou tentar)
				}else{
					printf("\n %d ", &bloco1.valor); //1 espaço e 1 linha, para centralizar(ou tentar)
				}
        }
